...
const API_SECRET_KEY =
  process.env.API_SECRET_KEY || crypto.randomBytes(32).toString("hex");
process.env.API_SECRET_KEY = API_SECRET_KEY;

// Create a permanent token for the application instance
const APP_INSTANCE_TOKEN =
  process.env.APP_INSTANCE_TOKEN ||
...
app.use("/api", (req, res, next) => {
  ...
  if (!isBrowserRequest) {
    console.log(
      `API access from non-browser: ${clientIp} - ${userAgent} - ${req.method} ${req.path}`,
    );
  }
  ...
  const appInstanceId =
    process.env.REPL_ID || process.env.APP_INSTANCE_ID || "";
  ...
  const allowedDomains = ["localhost:5000", req.headers.host || ""];
  ...
  if (!isBrowserRequest) {
    ...
    if (isSensitiveEndpoint && !hasValidApiKey && !hasValidAppToken) {
      ...
      console.warn(
        `Unauthorized API access attempt to sensitive endpoint: ${req.method} ${req.path} from IP: ${clientIp}`,
      );
      return res
        .status(403)
        .json({
          message: "Access denied. API key required for this endpoint.",
        });
    }
    ...
    if (!hasValidApiKey && !hasValidAppToken) {
      console.log(
        `External API access allowed (for demo purposes): ${req.method} ${req.path} from IP: ${clientIp}`,
      );
    }
  } else {
    ...
    if (
      ...
      console.warn(
        `Suspicious referer: ${refererDomain}, Expected one of: ${allowedDomains.join(", ")}`,
      );
    }
  }
...
(async () => {
  ...
  // More direct approach to serving static files from public directory
  app.use(express.static(path.join(process.cwd(), "public")));

  // Add middleware to log requests for debugging when accessing content files
  app.use("/content", (req, res, next) => {
    // Log but don't interfere with request handling
    console.log(`Content access: ${req.path}`);
    console.log(
      `Serving from: ${path.join(process.cwd(), "public", "content", req.path)}`,
    );
    next();
  });
